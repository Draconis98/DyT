...
import utils
from datasets_process import build_dataset
from dynamic_sigmoid import convert_ln_to_dys
from dynamic_sigmoid_zero_centered import convert_ln_to_dys_zero_centered
from dynamic_sigmoid_zero_scaled import convert_ln_to_dys_zero_scaled
...
def get_args_parser():
    ...
    parser.add_argument("--dynamic_sigmoid_zero_centered", type=str2bool, default=False)
    parser.add_argument("--dynamic_sigmoid_zero_scaled", type=str2bool, default=False)
...
def main(args):
    ...
    if (
        args.dynamic_tanh
        and args.dynamic_sigmoid
        and args.dynamic_sigmoid_zero_centered
        and args.dynamic_sigmoid_zero_scaled
    ):
        raise ValueError(
            "dynamic_tanh, dynamic_sigmoid, dynamic_sigmoid_zero_centered, and dynamic_sigmoid_zero_scaled cannot be used together"
        )
    if args.dynamic_tanh:
        model = convert_ln_to_dyt(model)
    if args.dynamic_sigmoid:
        model = convert_ln_to_dys(model)
    if args.dynamic_sigmoid_zero_centered:
        model = convert_ln_to_dys_zero_centered(model)
    if args.dynamic_sigmoid_zero_scaled:
        model = convert_ln_to_dys_zero_scaled(model)
